...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        organizations = [("ГРЧЦ", OrganizationType.GRFC)]
        organizations.extend((name, OrganizationType.UNIVERSITY) for name in university_organizations)
        organizations.extend((operator_name, OrganizationType.MOBILE_OPERATOR) for operator_name in operator_names)

        for name, org_type in organizations:
            organization, created = Organization.objects.get_or_create(
                name=name,
                type=org_type,
                defaults={"city": None, "is_active": True},
            )
            if not created and (organization.city_id or not organization.is_active):
                organization.city = None
                organization.is_active = True
                organization.save(update_fields=["city", "is_active", "updated_at"])
